    ...
    <div className="downloader-shell mt-8 w-full max-w-5xl text-left">
      ...
      {media && <><section className="result-card mt-6 overflow-hidden rounded-xl border p-3 sm:p-5" aria-label={`Analyzed ${media.media_type}`}>
        ...
        <div className="mt-4 border-t border-white/[0.07] px-1 pb-1 pt-4">
          ...
          })}</div></section><section aria-labelledby="audio-download-title" data-testid="audio-card" className="rounded-xl border border-[#2389ff]/20 bg-[#1682ff]/[0.045] p-3 sm:p-4"><p className="section-label !text-[0.66rem]">Audio</p><h3 id="audio-download-title" className="mt-1 text-sm font-semibold text-white/90">{platformName} audio</h3><p className="mt-2 text-xs leading-5 text-white/45"><span className="font-medium text-white/65">Source audio:</span> {sourceAudioDetails}</p>{originalAudioQuality && (() => { const key = "quality:audio"; const phase = downloadPhases[key]; const preparing = phase === "preparing"; const size = formatEstimatedSize(originalAudioQuality.estimated_size_bytes); return <button type="button" disabled={preparing} onClick={() => handleQuality("audio")} data-testid="original-audio-download-button" className="mt-3 flex min-h-11 w-full items-center justify-center gap-2 rounded-lg border border-white/[0.09] bg-white/[0.035] px-3 py-2.5 text-sm font-medium text-white/70 transition hover:border-white/20 hover:bg-white/[0.06] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#65c9ff] disabled:cursor-wait disabled:opacity-60"><StableDownloadAction loading={preparing} label={preparing ? "Preparing original audio..." : phase === "ready" ? "Original audio ready" : phase === "started" ? "Download started" : "Original / Best Audio"} details={[originalAudioQuality.container, size, "No MP3 transcode"].filter(Boolean).join(" · ")} testId="original-audio-action" /></button>; })()}{mp3Quality ? (() => { const key = "quality:mp3"; const phase = downloadPhases[key]; const preparing = phase === "preparing"; const label = preparing ? "Preparing MP3..." : phase === "ready" ? "MP3 ready" : phase === "started" ? "Download started" : `Download MP3 · ${mp3Bitrate} kbps output`; return <><fieldset disabled={preparing} className="mt-3" data-testid="mp3-bitrate-selector"><legend className="text-xs font-medium text-white/65">MP3 output bitrate</legend><div className="mt-2 grid grid-cols-2 gap-1.5">{MP3_BITRATE_OPTIONS.map((option) => { const selected = option.value === mp3Bitrate; const description = recommendedBitrate === option.value ? "Recommended" : option.description; return <button key={option.value} type="button" aria-pressed={selected} onClick={() => setMp3Bitrate(option.value)} className={`min-h-14 rounded-lg border px-1.5 py-2 text-center transition focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#65c9ff] disabled:cursor-wait disabled:opacity-55 ${selected ? "border-[#45b7ff]/55 bg-[#1682ff]/20 text-white shadow-[inset_0_0_0_1px_rgba(104,211,255,0.08)]" : "border-white/[0.08] bg-white/[0.025] text-white/55 hover:border-white/20 hover:bg-white/[0.05]"}`}><span className="block text-[0.68rem] font-semibold leading-4 sm:text-xs">{option.label}</span><span className={`mt-0.5 block text-[0.58rem] leading-3 sm:text-[0.65rem] ${selected ? "text-[#7fd5ff]" : "text-white/30"}`}>{description}</span></button>; })}</div></fieldset><p className="mt-2 text-[0.65rem] leading-4 text-white/30">Higher MP3 output bitrates increase file size but cannot restore quality that isn&apos;t present in the source.</p><button type="button" disabled={preparing} onClick={() => handleQuality("mp3")} data-testid="mp3-download-button" className="mt-3 flex min-h-12 w-full items-center justify-center gap-2 rounded-lg bg-gradient-to-r from-[#1478ff] to-[#1e56f5] px-4 py-3 text-sm font-semibold text-white transition hover:brightness-110 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#65c9ff] disabled:cursor-wait disabled:opacity-70"><StableDownloadAction loading={preparing} label={label} testId="mp3-action" /></button><span className="sr-only" aria-live="polite" data-testid="mp3-status">{phase ? label : ""}</span></>; })() : <p className="mt-4 rounded-lg border border-white/[0.07] px-3 py-2.5 text-xs text-white/35">MP3 is not available for this {platformName} post.</p>}</section></div> : gallery?.media_type === "image" ? <><div><p className="section-label !text-[0.66rem]">{platformName} photo</p><h3 className="mt-1 text-sm font-semibold text-white/90">Download image</h3></div><button type="button" disabled={downloadPhases.all === "preparing"} onClick={handleAll} className="mt-3 flex min-h-12 w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-[#1478ff] to-[#1e56f5] px-5 py-3 text-sm font-semibold text-white shadow-[0_10px_30px_rgba(20,120,255,0.2)] transition hover:brightness-110 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#65c9ff] disabled:cursor-wait disabled:opacity-70 sm:w-auto"><StableDownloadAction loading={downloadPhases.all === "preparing"} label={downloadPhases.all === "preparing" ? "Preparing..." : downloadPhases.all === "ready" ? "Download ready" : downloadPhases.all === "started" ? "Download started" : "Download Image"} testId="image-download" /></button></> : gallery ? <>
